...
ClaudeManager 采用**统一作用域（Scope）管理**，在顶部栏一键切换，贯穿所有配置类页面：

- **🌍 全局模式**（默认）：所有配置页操作 `~/.claude/` 下的用户级配置文件
- **📁 项目级模式**：点击顶部「项目级」选择项目目录后，所有配置页自动操作该项目 `.claude/` 下的配置

> 选择项目目录后，MCP 服务器、Hooks、Skills、自定义命令、配置、市场 会**自动同步切换到项目作用域**，所见即所配。顶部栏显示当前项目名，可随时「切换」或回到「全局」。
